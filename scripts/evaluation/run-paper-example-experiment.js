...
async function main() {
  ...
  await fs.writeFile(summaryPath, [
    ...
    "",
    "| Network condition | Gas price | Deployment + mint gas | Estimated ETH cost | Estimated USD cost |",
    "| --- | ---: | ---: | ---: | ---: |",
    ...scenarioCosts.map((scenario) => `| ${scenario.gwei === 10 ? "Low congestion" : scenario.gwei === 30 ? "Typical congestion" : "High congestion"} | ${scenario.gwei} gwei | ${deploymentGas} | ${scenario.eth.toFixed(6)} | ${scenario.usd === null ? "N/A" : `$${scenario.usd.toFixed(2)}`} |`),
    "",
    "## Phase measurements",
    "",
